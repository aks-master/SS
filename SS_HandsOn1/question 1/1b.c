...
Roll#: MT2024006
1(b) Create a hard link using system call
...
int main(){
printf("C program to create hardlink using link system call\n");
printf("First enter file name then hardlink name\n");
char filename[30];
char hardlink[30];
scanf("%s %s",filename,hardlink);
if(link(filename,hardlink)==0){
printf("hardlink created successfully\n");
}
else{
printf("Cannot create hardlink\n");
perror("Error: ");
}
return 0;
}
/* note
Run the program, enter an existing filename and corresponding hardlink name, after program finishes successfully run < ls -l >.
Check for your hardlink. 
eg. is given below
total 88
