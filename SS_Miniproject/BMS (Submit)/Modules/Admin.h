#define ADMINNAME "aks"
#define PASSWORD "aks123"
...
label1:
    ...
    else
    {
        ...
        write(connectionFD, writeBuffer, sizeof(writeBuffer));
        read(connectionFD, readBuffer, sizeof(readBuffer));
        goto label1; // so that we can ask credentials again.
    }
...
/*****function to add new bank employee*****/
int addEmployee(int connectionFD)
{
    struct Employee emp;
    ...
    // Employee FirstName
    bzero(writeBuffer, sizeof(writeBuffer));
    strcpy(writeBuffer, "Enter employee Name: ");
    write(connectionFD, writeBuffer, sizeof(writeBuffer));
...
/*****function to modify customer/employee*****/
void modifyCE(int connectionFD, int modifyChoice)
{
    if (modifyChoice == 1)
    {
    ...
    else if (modifyChoice == 2)
    {
        ...
        if (sourceFound)
        {
            char newName[20];
            bzero(writeBuffer, sizeof(writeBuffer));
            strcpy(writeBuffer, "Enter New Name(dont enter space): ");
            write(connectionFD, writeBuffer, sizeof(writeBuffer));
...
void manageRole(int connectionFD)
{
    ...
    while (read(file, &emp, sizeof(emp)) != 0)
    {
        if (emp.empID == id)
        {
            ...
            {
                printf("Admin made %d regular employee\n", id);
                emp.role = 1;
                write(file, &emp, sizeof(emp));
            }
            close(file);
