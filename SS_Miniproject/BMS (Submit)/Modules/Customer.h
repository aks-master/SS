...
label1:
    ...
    else
    {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        case 6:
                            ...
                            {
                                bzero(writeBuffer, sizeof(writeBuffer));
                                strcpy(writeBuffer, "Password changed successfully. login again....^");
                                write(connectionFD, writeBuffer, sizeof(writeBuffer));
                                read(connectionFD, readBuffer, sizeof(readBuffer));
                            }
                            logout(connectionFD, accountNumber);
                            goto label1; // so that we can login again.
                        case 7:
                            // View Transaction
        ...
        else
        {
            ...
            write(connectionFD, writeBuffer, sizeof(writeBuffer));
            read(connectionFD, readBuffer, sizeof(readBuffer));
            goto label1; // so that we can ask for credentials again.
        }
    }
}
...
void transferFunds(int connectionFD, int sourceAccount, int destAccount, float amount)
{
    ...
    if (dstOffset == -1)
    {
        ...
        strcpy(writeBuffer, "account not exist.^");
        write(connectionFD, writeBuffer, sizeof(writeBuffer));
        read(connectionFD, readBuffer, sizeof(readBuffer));
        return;
    }
...
void addFeedback(int connectionFD)
{
    ...
    {
    case 5:
        strcpy(fb.feedback, "Excellent (5 stars)");
        break;
    case 4:
        strcpy(fb.feedback, "Good (4 stars)");
        break;
    case 3:
        strcpy(fb.feedback, "OK(3 stars)");
        break;
    case 2:
        strcpy(fb.feedback, "can improve (2 stars)");
        break;
    case 1:
        strcpy(fb.feedback, "BAD (1 star)");
        break;
    case 0:
        strcpy(fb.feedback, "Very BAD (0 stars)");
        break;
    default:
        strcpy(fb.feedback, "contact cutomer");
    }
